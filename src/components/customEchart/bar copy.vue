  ...
  <div>
    <div class="data-per-second">
      <div v-for="(i, index) in timeS" class="div-dom" :key="index">
        ...
        <p v-if="i.info">
          <span v-for="(k, n) in i.info" class="icons" :style="infoStyle(i, k, n, index)" :key="n">
...
<script setup lang="ts">
  ...
  const infoStyle = computed(() => {
    return (parent: TimeInfo, item: ListDtoSubItem, index: number, parentIndex: number) => {
      let w = 0;
      if (parent.info) {
        let n = 0;
        ...
        if (n > 1) w = [29, 30].includes(item.type) ? index : index / 5;
      }
      let l = item.time - Number(parent.time) + w;
      console.log(l, '----lllll', widthVlaue.value);

      return `position: absolute;left:${widthVlaue.value * l - 1}px;top:${
        item.position == 1 ? 20 : 125
      }px`;
    };
  });
  ...
  const x = computed(() => {
    return (index: number) => {
      if (index == 0) return 1;
      const n = widthVlaue.value * index - 1;
      return n + index;
    };
  });
  ...
  .data-per-second {
    ...
    div {
      ...
      &:after {
        content: '';
        height: 76px;
        // border-right: 1px solid #eceff4; //TODO
